...
begin
  ...
  begin
    ...
    i:= 0;
    end_of_arr:= false;
    while sorted = true and end_of_arr = false do
    begin
      check:= nums_temp[i, 0];
      for i2:= 1 to length(nums_temp[i]) - 1 do
      begin
        if check <> nums_temp[i, i2] then
          sorted:= false;
      end;
      inc(i);
      if i = length(nums_temp) then
        end_of_arr:= true;
    end;
    nums_main:= nums_temp;
  end;
